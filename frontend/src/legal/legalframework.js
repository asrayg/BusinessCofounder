...
const LegalFramework = () => {
  ...
  // Function to fetch the legal framework from the API
  const fetchLegalFramework = async () => {
    ...
    if (businessData) {
      try {
        const response = await fetch('http://127.0.0.1:5000/lgegal', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        // Assuming the API returns a legal framework text under 'content'
        const legalTextFromAPI = data.content[0]?.text || 'Error: Could not generate legal framework.';
        
        // Update the legalText state with the API response
        setLegalText(legalTextFromAPI);

      } catch (error) {
        console.error('Error fetching legal framework:', error);
        setLegalText('Error: Could not fetch legal framework.');
      }
    } else {
      console.error('No business data found in localStorage');
      setLegalText('Error: No business data found in localStorage.');
    }
  };
  ...
  return (
    <div>
      ...
      <div className="legal-framework-container">
        ...
        {/* Button to fetch updated legal info */}
        <button className="fetch-api-button" onClick={fetchLegalFramework}>Fetch Updated Legal Info</button>
      </div>
    </div>
  );
};
